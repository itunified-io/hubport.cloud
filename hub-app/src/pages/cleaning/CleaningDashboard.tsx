...
import {
  Sparkles, Users, Calendar, RotateCcw, Check, X, Leaf,
} from "lucide-react";
...
export function CleaningDashboard() {
  ...
  const canManage = can("manage:cleaning");

  const [duties, setDuties] = useState<CleaningDuty[]>([]);
  const [gardenDuties, setGardenDuties] = useState<GardenDuty[]>([]);
  const [schedules, setSchedules] = useState<Schedule[]>([]);
  const [_groups, setGroups] = useState<ServiceGroup[]>([]);
  ...
  const fetchAll = async () => {
    try {
      const [dRes, gRes, sRes, grRes] = await Promise.all([
        fetch(`${apiUrl}/cleaning/duties`, { headers }),
        fetch(`${apiUrl}/cleaning/garden`, { headers }),
        fetch(`${apiUrl}/cleaning/schedules`, { headers }),
        fetch(`${apiUrl}/service-groups`, { headers }),
      ]);
      if (dRes.ok) setDuties(await dRes.json() as CleaningDuty[]);
      if (gRes.ok) setGardenDuties(await gRes.json() as GardenDuty[]);
      if (sRes.ok) setSchedules(await sRes.json() as Schedule[]);
      if (grRes.ok) setGroups(await grRes.json() as ServiceGroup[]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const seedDefaults = async () => {
    await fetch(`${apiUrl}/cleaning/seed`, { headers });
  ...
  const generateSchedule = async () => {
    ...
    await fetch(`${apiUrl}/cleaning/schedules/generate`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        dutyId: genDutyId,
  ...
  const updateStatus = async (scheduleId: string, status: string) => {
    await fetch(`${apiUrl}/cleaning/schedules/${scheduleId}`, {
      method: "PUT",
      headers,
  ...
  // If no duties or groups, show setup prompt
  if (duties.length === 0 && gardenDuties.length === 0) {
    return (
      <div className="space-y-6">
        <h1 className="text-xl font-semibold text-[var(--text)]">
          <FormattedMessage id="nav.cleaning" />
        </h1>
        <div className="flex flex-col items-center justify-center py-16 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-1)]">
          ...
          <p className="text-sm text-[var(--text-muted)] mb-4">
            <FormattedMessage id="cleaning.empty" />
          </p>
          {canManage && (
            <button onClick={seedDefaults} className="px-4 py-2 bg-[var(--amber)] text-black text-sm font-semibold rounded-[var(--radius-sm)] hover:bg-[var(--amber-light)] transition-colors cursor-pointer">
              <FormattedMessage id="cleaning.seed" />
            </button>
          )}
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {canManage && (
          <div className="flex gap-2">
            ...
            >
              <RotateCcw size={14} />
              <FormattedMessage id="cleaning.generate" />
            </button>
          </div>
        )}
      </div>
      ...
      {showGenerator && canManage && (
        <div className="p-4 border border-[var(--amber)] border-opacity-30 rounded-[var(--radius)] bg-[var(--bg-1)] space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            <select
              value={genDutyId}
              onChange={(e) => setGenDutyId(e.target.value)}
              className="px-3 py-1.5 text-sm bg-[var(--input)] border border-[var(--border)] rounded-[var(--radius-sm)] text-[var(--text)]"
            >
            ...
            <select
              value={genFreq}
              onChange={(e) => setGenFreq(e.target.value as typeof genFreq)}
              className="px-3 py-1.5 text-sm bg-[var(--input)] border border-[var(--border)] rounded-[var(--radius-sm)] text-[var(--text)]"
            >
              <option value="weekly">{intl.formatMessage({ id: "cleaning.frequency.weekly" })}</option>
              <option value="biweekly">{intl.formatMessage({ id: "cleaning.frequency.biweekly" })}</option>
              <option value="monthly">{intl.formatMessage({ id: "cleaning.frequency.monthly" })}</option>
            </select>
            <select
              value={genMonths}
              ...
              className="px-3 py-1.5 text-sm bg-[var(--input)] border border-[var(--border)] rounded-[var(--radius-sm)] text-[var(--text)]"
            >
              <option value={3}>3 {intl.formatMessage({ id: "cleaning.months" })}</option>
              <option value={6}>6 {intl.formatMessage({ id: "cleaning.months" })}</option>
              <option value={12}>12 {intl.formatMessage({ id: "cleaning.months" })}</option>
            </select>
            <button
              onClick={generateSchedule}
              disabled={!genDutyId}
              className="px-4 py-1.5 text-sm font-semibold bg-[var(--amber)] text-black rounded-[var(--radius-sm)] hover:bg-[var(--amber-light)] disabled:bg-[var(--border)] disabled:text-[var(--text-muted)] transition-colors cursor-pointer"
            >
              <FormattedMessage id="cleaning.generate.run" />
            </button>
          </div>
        </div>
      )}
      ...
      <div className="p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-1)] space-y-4">
        <h2 className="text-sm font-medium text-[var(--text)] flex items-center gap-2">
          ...
          <FormattedMessage id="cleaning.title" />
        </h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-left text-xs text-[var(--text-muted)]">
                <th className="px-4 py-3 font-medium"><FormattedMessage id="cleaning.schedule.date" /></th>
                <th className="px-4 py-3 font-medium"><FormattedMessage id="cleaning.schedule.duty" /></th>
                <th className="px-4 py-3 font-medium hidden sm:table-cell"><FormattedMessage id="cleaning.schedule.group" /></th>
                <th className="px-4 py-3 font-medium"><FormattedMessage id="cleaning.schedule.status" /></th>
                  ...
                  {canManage && (
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {s.status !== "completed" && (
                          <button onClick={() => updateStatus(s.id, "completed")} className="p-1 text-[var(--green)] hover:bg-[var(--glass)] rounded cursor-pointer">
                            <Check size={14} />
                          </button>
                        )}
                        {s.status !== "skipped" && (
                          <button onClick={() => updateStatus(s.id, "skipped")} className="p-1 text-[var(--text-muted)] hover:bg-[var(--glass)] rounded cursor-pointer">
                            <X size={14} />
                          </button>
                        )}
                      </div>
      ...
      <div className="p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-1)] space-y-4">
        <h2 className="text-sm font-medium text-[var(--text)] flex items-center gap-2">
          ...
          <FormattedMessage id="cleaning.garden.title" />
        </h2>
            ...
            <div key={d.id} className="p-3 border border-[var(--border)] rounded-[var(--radius-sm)] bg-[var(--bg)]">
              ...
              {d.members.length > 0 ? (
                ...
                <p className="text-xs text-[var(--text-muted)]">
                  <FormattedMessage id="cleaning.garden.noMembers" />
                </p>
              )}
            </div>
          ))}
